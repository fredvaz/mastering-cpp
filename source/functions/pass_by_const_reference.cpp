...
The C++20 Masterclass: Pass by Const reference (C++)
...
void say_age(const int &age) // Parameter passed by Const reference
                             // Constant reference NOT allows to modify the data
{
    //++age; // Compiler error: increment of read-only reference 'age'
    cout << "Hello , you are " << age << " years old! &age : " << &age << endl; // 70
}
...
int main()
{
    ...
    int age{70};
    cout << "age (before call): " << age << " &age: " << &age << endl; // 70

    say_age(age);
    cout << "age (after call): " << age << " &age: " << &age << endl; // 70
