...
void BoxContainer::add(const value_type &item)
{
	if (m_size == m_capacity)
		expand(m_size + EXPAND_STEPS); // Let's expand in increments of 5 to optimize calling expand() method
	m_items[m_size] = item;
	++m_size;
}
...
bool BoxContainer::remove_item(const value_type &item)
{
	...
	// Find the target item by the Indexes
	for (size_t i{0}; i < m_size; ++i)
	{
		if (m_items[i] == item)
		{
			index = i;
			break; // No need for the loop to go on once the Index found
		}
	}
...
size_t BoxContainer::remove_all(const value_type &item)
{
	...
	bool removed = remove_item(item); // Uses the method remove_item()
	if (removed)
		++remove_count;
