...
/*
    !Note: #define EXAMPLE_N
            ...
            *Using the std::make_unique<type> keyword we can avoid the use of new keyword
            !We can not copy the unique pointers
            *But we move ownership
*/
...
int main()
{
    ...
    //* Stack variables
    {
        ...
        dog1.print_dog();
    } //* The Destructor is called in the scope end
    std::cout << "Hitting outside scope" << std::endl;

    std::cout << "------------------------------ EXAMPLE_2 -------------------------------------" << std::endl;

    //* Using raw pointers: Remember to manually release memory, if you don't relase , you leak memory
    ...
    //! If you go out of scope without releasing (deleting) p_dog2 and p_int1 you'll have leaked memory
    ...
    {
        Dog *p_dog_3 = new Dog("Dog3");         //! Yet using the Raw Pointer
